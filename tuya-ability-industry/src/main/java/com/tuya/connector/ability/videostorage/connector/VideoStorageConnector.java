...
public interface VideoStorageConnector{
    ...
    List<HlsStreamTimeline> streamTimeline(@Path("uid")String uid, @Path("device_id")String deviceId);
    ...
    HlsKey streamKey(@Path("uid")String uid, @Path("device_id")String deviceId, @Path("magic")String magic);

    /**
     * 获取云存储录像片段的播放资源
     * */
    @GET("/v1.0/users/{uid}/devices/{device_id}/storage/stream/hls")
    List<HlsLocation> streamHls(@Path("uid")String uid, @Path("device_id")String deviceId);
}
