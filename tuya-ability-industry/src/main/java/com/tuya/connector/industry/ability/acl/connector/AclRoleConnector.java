...
import com.tuya.connector.industry.ability.acl.model.AclRolesPageParam;
...
public interface AclRoleConnector {
    ...
    AclRole queryRoleByCode(@Path("space_id") String spaceId, @Path("role_code") String roleCode);
...
/*    @POST("/v1.0/iot-03/idaas/get-batch-role")
    ...
    List<AclRole> queryRolesByUid(@Path("space_id") String spaceId,
                                   @Path("uid") String uid);

    @POST("/v1.0/iot-03/idaas/spaces/{space_id}/page-role")
    AclPageResult<AclRole> queryPagingRoles(@Path("space_id") String spaceId,
                                                @Body AclRolesPageParam aclRolesPageParam);
}
