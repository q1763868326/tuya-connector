...
import com.tuya.connector.industry.ability.acl.model.AclSpace;
 ...
 * <p>
 * about iot developer, project, application.
 * a iot developer can own many project. (one to many)
 * a project have some applications. (one to many)
 * a iot developer identity by iotuid. (or named uid)
 * a project identity by a tuple(ak,sk). (ak also named clientId,sk also named secret key)
 * <p>
 * acl space(or named idaas space, permission space) has three shared scope.
 * <p>
 * the 1st is developer scope.(authentication=1)
 * this scope shared data in the developer's all projects and all applications.
 * <p>
 * the 2nd is project scope.(authentication=2)
 * this scope shared data in the project's all application.
 * <p>
 * the 3rd is application.(authentication=3)
 * data in this scope only can be accessed by the application which apply it.
 * <p>
 * most often, we just need apply the project scope.
 * so, when we apply the acl space, we use authentication=2
 *
 * @author benguan.zhou@tuya.com
 * @description
 * @date 2021/05/31
 */
public interface AclSpaceConnector {

    @POST("/v1.0/iot-03/idaas/spaces")
    String applySpace(@Body AclSpace aclSpace);

    @GET("/v1.0/iot-03/idaas/spaces")
    AclSpace querySpace(@Query("space_group") String spaceGroup,
                        @Query("space_code") String spaceCode);
}
